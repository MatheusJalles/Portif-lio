window.addEventListener('load', () => {
  ...
  function atualizar() { track.style.transform = `translateX(-${index * width}px)`; }

  next.onclick = () => { index = (index + 1) % slides.length; atualizar(); }
  prev.onclick = () => { index = (index - 1 + slides.length) % slides.length; atualizar(); }

  /* LOOP AUTOMÁTICO DO CARROSSEL */
  setInterval(() => { index = (index + 1) % slides.length; atualizar(); }, 3000);

  /* =====================
     MÚSICA COM CONTROLE DE VOLUME
  ====================== */
